...
function registrar(fk_usuario, pontos) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function registrar():", fk_usuario, pontos);
    
    
    var instrucao = `
        INSERT INTO classificacao(fk_usuario,fk_ponto,dataHora) VALUES (${fk_usuario},${pontos}, now());
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
...
function cadastrar(usuario, email, senha) {
    console.log("ACESSEI O USUARIO MODEL \n \n\t\t >> Se aqui der erro de 'Error: connect ECONNREFUSED',\n \t\t >> verifique suas credenciais de acesso ao banco\n \t\t >> e se o servidor de seu BD está rodando corretamente. \n\n function cadastrar():", usuario,usuario,email);
    var instrucao = `
...
function classificar() {
    ...
    select u.usuario as "usuario", 
           po.ponto "pontos",
           date_format(c.dataHora, '%d/%m/%y') as "data"
